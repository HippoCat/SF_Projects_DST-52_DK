...
1. Качество кода (соблюдение стандартов оформления pep-8, комментирование кода, наличие выводов ). Оформление проекта на Github— 3 балла.
2. Качество разведывательного анализа данных (визуализация, очистка данных, работа с выбросами) — 3 балла.
3. Работа с признаками (выбор признаков, генерация новых признаков) — 3 балла.
4. Качество решения: Результат метрики ROC AUC — 3 балла.
5. Качество решения: обоснованность выводов и общая логика решения — 3 балла.
6. Работа в команде — 3 балла.
7. Настройка гиперпараметров — 3 балла.
8. Работа сдана к дедлайну — 3 балла. 
...
3. Проект выполнял локально на своем компе в Jupyter, файлы 'submission_r**.csv' сохранял локально, на Kaggle загружал руками.
4. По завершении проекта загрузил Notebook с решением на GitHub и на Kaggle.
5. Представленный ноутбук практически полностью оригинальный. То есть в другие ноутбуки я подгладывал конечно, но в результате сделал свой собственный с нуля.
6. Большая благодарность Lenka Vinogradova (https://www.kaggle.com/lenkavinogradova) из ноутбука которой почерпнул много интересного.

### Проект занял около 50 часов. Многовато, но это не только чистый кодинг, но и reading, thinking, asking и тупинг.
...
#### Итак, напишем функцию предобработки данных для наивной модели, предобработка минимальная:
 ...
 - это будет самое днище предобработки для наивной модели - от него будем отталкиваться.
...
### 2.2. Модель №2 - с улучшенной предобработкой - submission_r02
...
### 2.3. Модель №3 с улучшенной предобработкой и подбором признаков - submission_r03
...
#### Смотрели на корреляцию и значитмость признаков:
...
#### Признаки 'education' и 'region_rating' тупо отправили в список категориальных. ROC AUC подрос на 0.01 и поднялись на 40 позиций в Kaggle (!)
...
### 2.4. Модель №4 с дальнейшими улучшениями предобработки и подбором признаков - submission_r04
...
### 2.5. Игры разума с параметрами - submission_r05_hyper

#### Берём вариант подготовки данных №4 и перебираем возможные (не все) гиперпараметры модели LogisticRegression() 
На практике убеждаемся, что не все параметры совсестимы друг с другом. Перебирал параметры руками.

#### Типичный результат - разница существенна только в 4 знаке:
...
### 2.6. и 2.7 Пробую самодельный и SMOTE oversampling и применяю модель №4 без гиперпараметров - submission_r06_oversample & submission_r07_smote
...
#### Выводы относительно модели с самодельным oversampling:
...
- На месте банкира, я бы предпочел работать с такой более сбалансированной матрицей.

### На этом закончил, хотя можно улучшать до бесконечности

__Лучшая позиция на Kaggle: 64 Dmitry Kuznetsov DST-52 score=0.73787__
...
1. Уделять как минимум полную неделю для проекта, а лучше две;
2. Надо учить матчасть, учиться быстро разбираться с новыми методами и подходами.
3. Надо набирать практику и "набивать руку", чтобы как можно больше вещей делать с лёту, не задумываясь.
